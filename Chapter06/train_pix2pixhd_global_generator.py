...
from utils import iterate_minibatches_cityscapes, log_images
from utils import log_losses_pix2pixhd, save_model
...
def train(data_folderpath='data/cityscapes', image_size=256, ndf=32, ngf=64,
          lr_d=2e-4, lr_g=2e-4, n_feat_channels=3, use_edges=True,
          n_iterations=int(1e6), batch_size=1, iters_per_checkpoint=100,
          n_checkpoint_samples=4, feature_matching_weight=10,
          out_dir='pix2pixhd'):
    ...
    G_model = Model(
        ...
        outputs=[D0_fake, D1_fake, D2_fake, fake_samples, fake_samples,
                 fake_samples])
